...
export default function Navbar() {
    ...
    const navLinks = (
        <>
            <NavLink
                to="/"
                className={({ isActive }: { isActive: boolean }) => `text-white/80 font-medium transition-all duration-300 relative group px-3 sm:px-4 py-2 mb-2 rounded-full text-sm sm:text-base ${isActive ? 'text-white bg-white/5' : 'hover:text-white hover:bg-white/5'}`}
            >
